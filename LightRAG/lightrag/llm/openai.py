...
async def deepseek_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    logger.debug("===== Sending Query to DeepSeek LLM =====")
    logger.debug(f"Model: {model}")
    logger.debug(f"Additional kwargs: {kwargs}")
    verbose_debug(f"Query: {prompt}")
    verbose_debug(f"System prompt: {system_prompt}")

    # Ensure proper encoding for all text content
    for message in messages:
        if isinstance(message.get("content"), str):
            # Ensure content is properly encoded as UTF-8
            message["content"] = safe_unicode_decode(message["content"].encode("utf-8"))

    try:
        if "response_format" in kwargs:
            response = await deepseek_client.beta.chat.completions.parse(
                model=model, messages=messages, **kwargs
            )
        else:
        ...
        logger.error(f"DeepSeek API Rate Limit Error: {e}")
        raise
    except APITimeoutError as e:
    ...
    if hasattr(response, "__aiter__"):
        ...
        if (
            ...
            raise InvalidResponseError("Invalid response from DeepSeek API")

        content = response.choices[0].message.content

        if not content or content.strip() == "":
            logger.error("Received empty content from DeepSeek API")
            raise InvalidResponseError("Received empty content from DeepSeek API")
